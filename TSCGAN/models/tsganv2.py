...
class TSCGAN(object):
    ...
    def graph(self, inputs, labels):
        ...
        # [batch_size, ]
        _, fake_prob = self.D(self.fake_data)
        logits, true_prob = self.D(inputs, reuse=True)
        self.logits_ = logits

        D_loss = -tf.reduce_mean(tf.log(true_prob) + tf.log(1 - fake_prob + 1e-8), name='real_fake_loss')
        ...
        self.D_loss_factor = tf.Variable(.3, trainable=False, name='D_loss_factor')

        D_loss = tf.add(classification_loss, self.D_loss_factor * D_loss, name="d_loss")

        new_discount_factor = self.D_loss_discounted_factor * self.D_loss_factor
        self.discount_D_loss_rate = tf.assign(self.D_loss_factor, new_discount_factor)
        self.set_discount_d_loss_fac_op = tf.assign(self.D_loss_factor, self.D_loss_factor_ph)
    ...
    def discount_d_loss_factor(self, discount_rate=0.99):
        """
        new_discount_factor = discount_rate* old_discount_factor
        """
        sess = tf.get_default_session()
        if sess is None:
            raise ValueError("in the with tf.Session() as sess block")
        sess.run(self.discount_D_loss_rate, feed_dict={self.D_loss_discounted_factor: discount_rate})

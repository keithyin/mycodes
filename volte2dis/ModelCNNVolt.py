...
class Model(object):
    ...
    def graph(self):
        ...
        pool1 = tf.nn.avg_pool(conv1,[1,4,1,1],[1,4,1,1],padding="SAME") # pooling层
        tf.summary.histogram("pool1",pool1)
        conv2W = tf.get_variable("conv2W", shape=[10, 1, 4, 4], dtype=tf.float32)
        conv2b = tf.get_variable("conv2b", shape=[4], dtype=tf.float32)
        tf.summary.histogram("conv2W", conv2W)
        conv2 = tf.nn.conv2d(pool1, conv2W, [1, 3, 1, 1], padding="SAME") #第二层卷积
        conv2 = tf.nn.bias_add(conv2, conv2b)

        conv2 = tf.maximum(0.6*conv2,conv2)
        pool2 = tf.nn.avg_pool(conv2, [1, 4, 1, 1], [1, 4, 1, 1], padding="SAME")#pooling层
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        if os.path.exists("checkpoint_cnn_v1/modelCNN"):
            saver.restore(sess, "checkpoint_cnn_v1/modelCNN")
        writer = tf.summary.FileWriter("checkpoint_cnn_v1", sess.graph)
